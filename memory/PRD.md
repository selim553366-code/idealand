...
- 2026-07: Bot team — 6 persona bots (Brand, Customer Insight, Ad Marketing, Financial, Developer, Ad Video) discuss the idea via streamed SSE messages with DISTINCT SVG ROBOT CHARACTERS (per-bot color gradient, antenna type, eye style, blink + bob animations) — no plain icons. Discussion texts stay VISIBLE in chat (user's explicit choice) and ALSO persist per project (discussion array) for portfolios. Every user message (new idea OR change request) triggers a team deliberation first; edits auto-proceed after the decision, new builds show the Team decision card. Each bot has a portfolio modal (team bar → click robot) aggregating its notes + suggestions across projects. After every build/edit a "Build report" card shows what was done + 1-2 next-step suggestion chips ONLY from the most relevant bots (clickable → sends as message). Video bot NEVER acts without approval; on approval produces a storyboard card (concept only, no video render API connected). Version history: every edit stored as a version, v1..vN chips in preview header restore any version (revert endpoint). Chat bars (homepage + studio) have file attach (5MB, text files injected into context) and Website/App type selector shown as a chip inside the bar. Questions flow replaced by team discussion.
