...
- 2026-07: Bot team — 6 persona bots (Brand, Customer Insight, Ad Marketing, Financial, Developer, Ad Video) discuss the idea via streamed SSE messages with gradient avatars and typing animation, then present a Team Plan card (Build it / Build + video ad). Video bot NEVER acts without approval; on approval produces a storyboard card (scenes + CTA — concept only, no video render API connected). Version history: every edit stored as a version, v1..vN chips in preview header restore any version (revert endpoint). Chat bars (homepage + studio) have file attach (5MB, text files injected into context) and Website/App type selector shown as a chip inside the bar. Questions flow replaced by team discussion.
